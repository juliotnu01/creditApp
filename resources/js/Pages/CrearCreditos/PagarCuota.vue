                ...
                if(iterator.has_many_compromisos){
                    ...
                    if(iterator.status == 2){
                        if(iterator.has_many_compromisos.length > 0){
                            ...
                            iterator.has_many_compromisos[iterator.has_many_compromisos.length -1].caducado = false
                        }
                    }
                    ...
                    <table class="table-auto w-full">
                        ...
                        <tbody class="text-sm divide-y divide-gray-100">
                            <tr v-for="(pago, p) in AmortizaiconesGenerales" :key='p'>
                                ...
                                <td class="p-2">
                                    <div class="text-center font-medium" v-if="pago.has_many_compromisos.length > 0">
                                        <span v-if="pago.pagoCompromiso == false" :class="{ 'bg-green-700 text-white py-1 px-3 rounded-full flex justify-between gap-2  whitespace-no-wrap': pago.has_many_compromisos[pago.has_many_compromisos.length -1].caducado  == false, 
                                                        'bg-red-700 text-white py-1 px-3 rounded-full flex justify-between gap-2  whitespace-no-wrap': pago.has_many_compromisos[pago.has_many_compromisos.length -1].caducado  == true }">
                                        ...
                                        <span v-else class="bg-green-700 text-xs text-white py-1 px-3 rounded-full flex justify-between gap-2">
                                            <p> Compromiso pagado </p>  
                                            <svg @click.prevent="openModalViewCompromiso(pago)" width="24px" height="24px" viewBox="0 0 24 24" fill="none" class="text-white hover:cursor-pointer">
                                                <g id="style=stroke">
                                                    <g id="eye-open">
                                                        <path id="vector (Stroke)" fill-rule="evenodd" clip-rule="evenodd"
                    ...
                    <div class="overflow-x-auto p-2">
                        ...
                        <table class="table-auto w-full ">
                            ...
                            <tbody class="text-sm divide-y divide-gray-100">
                                ...
                                <td class="p-2">
                                    <div class="text-center font-medium whitespace-no-wrap" >
                                        <span v-if="pago.pagoCompromiso == false" :class="{'bg-green-700 text-white py-1 px-3 rounded-full flex justify-between gap-2': pago.caducado == false, 'bg-red-700 text-white py-1 px-3 rounded-full flex justify-between gap-2': pago.caducado == true }">
                                            {{ pago.caducado == false ? 'Activo' : 'Caducado' }}
                                        </span>
                                        <span v-else class="bg-green-700 text-white py-1 px-3 rounded-full flex justify-between gap-2">
                                            Compromiso pagado
                                        </span>
                                    </div>
                                </td>
